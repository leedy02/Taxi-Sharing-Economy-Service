...
def last_candidate(host_id, candidate_arr):
    ...
    id_route = []
    for i in range(len(candidate_arr)):
        guest_d = conn.select_d(candidate_arr[i][0])[0]
        guest_route = json.loads(conn.select_route_from_destination(candidate_arr[i][0])[0][0])
        guest_cost = guest_route['features'][0]['properties']['taxiFare']
        if candidate_arr[i][1] == True:
            bypass_route = t_api.bypass_route(host_d[0], host_d[1], guest_d[2], guest_d[3], guest_d[0],guest_d[1], host_d[2], host_d[3])
        else:
            bypass_route = t_api.bypass_route(host_d[0], host_d[1], host_d[2], host_d[3], guest_d[0],guest_d[1], guest_d[2], guest_d[3])
        ...
        if bypass_cost / (host_cost+guest_cost) < min_cost_rate:
            ...
            last_candidate = candidate_arr[i][0]
